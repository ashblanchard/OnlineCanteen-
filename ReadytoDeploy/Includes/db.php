...
// CAMPER & STAFF CRUD FUNCTIONS    
    ...
    // public function get_camper_balance_by_camper_id($camperid) {
    //  return $this->query("SELECT initialBalance FROM campers WHERE id = " . $camperid );
    //}
...
function get_seggie_type($type, $deposit, $balance) {
    
    if (strcmp($type, "Camper")== 0){
    echo get_new_balance($balance) ;
    }
    
else{
    echo get_new_deposit($deposit);
}
}
?>
